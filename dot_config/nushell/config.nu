...
alias c = claude
alias commit = claude -p '/caveman-commit Actually run the git commands now. Review staged and unstaged changes, run git add -A, then run git commit with a clear conventional commit message.' --model claude-haiku-4-5 --allowedTools 'Bash(git status)' 'Bash(git status *)' 'Bash(git diff)' 'Bash(git diff *)' 'Bash(git log)' 'Bash(git log *)' 'Bash(git add *)' 'Bash(git commit *)'
alias o = opencode
alias y = yazi
alias h = herdr
