...
local function syncPackages(ensurePacks)
  ...
  local function refreshCallback()
    ...
    vim.iter(ensurePacks):each(function(packName)
      ...
      if not masonReg.has_package(name) then return end
      local pack = masonReg.get_package(name)
      if pack:is_installed() then
        pcall(function()
          local latest = pack:get_latest_version() -- sync call (v2.1.0+)
          local current = pack:get_installed_version()
          if latest ~= current then install(pack, latest) end
        end)
      else
